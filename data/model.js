...
monoProvider.prototype.getCollection_mono= function(callback) {
  this.db.collection("mono", function(error, mono_collection) {
...
monoProvider.prototype.getCollection_user= function(callback) {
  this.db.collection("mono_user", function(error, mono_collection) {
    if( error ) callback(error);
    else callback(null, mono_collection);
  });
};
/*
monoProvider.prototype.getCollection_category= function(callback) {
	this.db.collection("mono_category", function(error, mono_collection) {
	    if( error ) callback(error);
	    else callback(null, mono_collection);
	});
};
...
monoProvider.prototype.getCollection_messanger = function(callback) {
	this.db.collection("mono_messanger", function(error, mono_collection) {
	    if( error ) callback(error);
	    else callback(null, mono_collection);
	});
};
...
//add new mono
monoProvider.prototype.insertMono = function(monodata,ses,callback) {
	//TODO <<<<
	//_lib.log(monodata.mono.imagedata.match(/data:image\/\(jpeg|png|jpg|bmp\);/)[0],"monodata.imagedata");
	...
	var cols = {
			...
			//img: {match:{req:1,msg:"valid img required"}},
			//img: {}, //<--- del???
			//imageurl:{},
			state: {req:1},
			city: {req:1},
			category: {req:1,match:{regx:/^[0-9a-zA-Z_]{24,24}$/,msg:"valid category required"},convert:lib.convertObjectId},
		    ...
		    },insert_p = {res:{writer_screen_name:ses.screen_name,writer_id:lib.convertObjectId(ses.uid),images:allImg,created_at:new Date()},msg:{}};
	
	for ( var k in cols) {
		
		//required
		if(cols[k].req && !data_mono[k]){
			insert_p.msg[k] = "["+k + "] : required";
		}
		if(!data_mono[k])continue;
		if(cols[k].match){
			if(cols[k].match.regx && !cols[k].match.regx.test(data_mono[k])) insert_p.msg[k] = cols[k].match.msg;
			else if(cols[k].match.arr && cols[k].match.arr.indexOf(data_mono[k]) === -1) insert_p.msg[k] = cols[k].match.msg;
			else if(cols[k].match.func && !cols[k].match.func(data_mono[k])) insert_p.msg[k] = cols[k].match.msg;
		}
		if(cols[k].len && cols[k].len[0] > data_mono[k].length && cols[k].len[1] < data_mono[k].length) insert_p.msg[k] = "["+k + "] : wrong length of characters";
		insert_p.res[k] = (cols[k].convert)?cols[k].convert(data_mono[k]):data_mono[k];
	}
	//error
		...
		//TODO : password sha1,add created_at,required
	      if( error ) callback(error);
	    	  ...
	    	  mono_collection.insert(insert_p.res, function(error,results) {
	    		  _lib.log(results[0]._id,"insert_mono");
